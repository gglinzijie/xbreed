...
make_rp<-function(sh_out,Male_founders,Female_founders,ng,litter_size,Selection,Training,saveAt,rp_output,Display) {
...
# Male_founders
	...
	if(Male_founders[,1]<1 | floor(Male_founders[,1])!=Male_founders[,1] ) {
	stop('\n','--- Number of selected males in argument "Male_founders" should be an integer grater than 1')
	}
	
	outforLD<-sh_out
	if(Male_founders[,2]<0 | floor(Male_founders[,2])!=Male_founders[,2] | Male_founders[,2]>(length(outforLD$output)-1)) {
	stop('\n','--- Generation index in argument "Male_founders" should be an integer in range 0<=x<=number of generations from function sample_hp/ make_rp')
	}
	...
	if(Male_founders[,1]>t_mad) {
	stop('\n','--- Number of males in argument "Male_founders" is more than number of males in generation: ',Male_founders[,2],' of previous population.',"\n",'Decrease number of selected males')
	}
...
# Female_founders
	...
	if(Female_founders[,1]<1 | floor(Female_founders[,1])!=Female_founders[,1] ) {
	stop('\n','--- Number of selected females in argument "Female_founders" should be an integer grater than 1')
	}
	
	outforLD<-sh_out
	if(Female_founders[,2]<0 | floor(Female_founders[,2])!=Female_founders[,2] | Female_founders[,2]>(length(outforLD$output)-1)) {
	stop('\n','--- Generation index in argument "Female_founders" should be an integer in range 0<=x<=number of generations from function sample_hp/ make_rp')
	}
	...
	if(Female_founders[,1]>t_mad) {
	stop('\n','--- Number of females in argument "Female_founders" is more than number of females in generation: ',Female_founders[,2],' of previous population.',"\n",'Decrease number of selected females')
	}
...
# dam data
my_internal_data1<-outforLD$output[[Females[,2]+1]]
my_internal_data<-my_internal_data1[[1]]
my_internal_data
